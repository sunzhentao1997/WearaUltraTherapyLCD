...
void setup_scr_Charge(lv_ui *ui)
{
	...
	ui->g_kb_Charge = lv_keyboard_create(ui->Charge);
	lv_obj_add_event_cb(ui->g_kb_Charge, kb_event_cb, LV_EVENT_ALL, NULL);
	lv_obj_add_flag(ui->g_kb_Charge, LV_OBJ_FLAG_HIDDEN);
	lv_obj_set_style_text_font(ui->g_kb_Charge, &lv_font_montserratMedium_16, LV_PART_MAIN|LV_STATE_DEFAULT);
	...
	lv_obj_set_pos(ui->Charge_label_1, 19, 14);
